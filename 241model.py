...
if __name__=='__main__':
    ...
    data_train = data[:-5]##训练数据
    #data_train = data_smooth(data_train)###平滑数据
    data_test = data[-5:]##测试数据
    ...
    model_tre = ARMA(adf_tre_diff,order=(7,2))
    ...
    if rdtest[1] > 0.05:#残差为白噪声
        ...
        ts.plot(color='red', label='Original')
        plt.legend(loc='best')
        plt.title('RMSE: %.4f'% np.sqrt(sum((log_recover1-ts)**2)/ts.size))
        plt.show()
    else:
